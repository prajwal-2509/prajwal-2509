# 👋 Hi, I'm Prajwal Hulle

![](https://komarev.com/ghpvc/?username=prajwal-2509\&color=blueviolet)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat\&logo=linkedin)](https://www.linkedin.com/in/prajwal-hulle-b726a5357)
[![HackerRank](https://img.shields.io/badge/HackerRank-green?style=flat\&logo=hackerrank)](https://www.hackerrank.com/profile/prajwalashokhul1)
[![Instagram](https://img.shields.io/badge/Instagram-black?style=flat\&logo=instagram)](https://www.instagram.com/_prjwal25/)

---

## 👨‍💻 About Me

Diploma graduate in Computer Technology from Government Polytechnic Solapur.
Focused on building **real systems** and understanding how things work internally.

* ⚛️ Exploring **Quantum Computing concepts**
* 🤖 Learning **Machine Learning & Data Science**
* ⚙️ Interested in **Backend Systems & Security**
* 🧠 Practicing **DSA & problem solving**

---

## 🚀 Currently

* Deepening **ML concepts using Python & Anaconda**
* Working on **real-world projects & automation**
* Improving **coding consistency (HackerRank)**

---

## 🛠️ Tech Stack

**Languages**
`Java` `Python` `JavaScript` `SQL` `C` `C++`

**Frontend**
`React` `HTML` `CSS` `Tailwind` `Bootstrap`

**Backend & Tools**
`Node.js` `Express.js` `Firebase` `Supabase` `JWT`

**Data & ML**
`NumPy` `Pandas` `Matplotlib` `Scikit-learn` `Anaconda`

**Databases**
`MySQL` `MongoDB` `Supabase DB`

**Tools**
`Git` `GitHub` `VS Code` `Postman`

---

## 🏆 GitHub Stats

![GitHub Stats](https://github-readme-stats-sigma-five.vercel.app/api?username=prajwal-2509&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=prajwal-2509&layout=compact&theme=tokyonight)
---

## 📈 Coding Practice

* 🧠 HackerRank → https://www.hackerrank.com/profile/prajwalashokhul1

---

## 📫 Connect

* 💼 LinkedIn → https://www.linkedin.com/in/prajwal-hulle-b726a5357
* 📧 Email → [prajwalashokhulle@gmail.com](mailto:prajwalashokhulle@gmail.com)
* 📸 Instagram → https://www.instagram.com/_prjwal25/

---

> I build to understand — not just to finish.
